...
class KnowledgeGraphEmbeddingModel(BaseEstimator, RegressorMixin, metaclass=ABCMeta):
    ...
    seed: int
        random seed
    verbose: int
        verbosity level. options are {0, 1.txt, 2}

    """

    def __init__(self, em_size=200, batch_size=128, nb_epochs=100, initialiser="xavier_uniform", nb_negs=2,
    ...
    def init_logging(self, verbose=0):
        ...
        if verbose == 0:
            ...
            self.log.critical('Unknown verbosity level (%d). Options are [0, 1.txt, 2]' % verbose)
